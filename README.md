...
- Without `ADMIN_PASSWORD`: `/admin` shows **Kitchen board offline** (APIs stay 404 — fail-closed). Fake-pay will not link to admin.
...
4. `/admin` — offline until `ADMIN_PASSWORD` is set in Vercel; then login → Mark ready.
